...
class Quiz(models.Model):  # Main model for quiz
    STATUS_CHOICES = (
        ('inactive', 'Inactive'),
        ('active', 'Active'),
        ('finished', 'Finished'),
    ...
    # objects = models.Manager()  # The default manager (required for front end)
    # active_quiz_objects = ActiveQuizManager()  # Custom manager for active quizzes
    
@receiver(pre_save, sender=Quiz)  # This is a signal that will be fired before saving the quiz
...
class Question(models.Model):  # Model for questions in quiz 
